...
AMQP_PORT = int(os.environ.get('AMQP_PORT', 5670))
...
AMQP_SPEC_FILE = os.environ.get('AMQP_SPEC_FILE', '/app/amqp0-9-1.xml')

EXPIRY_PERIOD = 60 * 60 * 24 * 7  # 7 days
redis_con = redis.Redis(host=REDIS_HOST, port=REDIS_PORT)


def decorate_key(key):
    return f"sms_cdr_consumer-{key}"


def push(key: str, value: dict, **kwargs) -> bool:
    return redis_con.set(decorate_key(key), json.dumps(value), **kwargs)


def get(key: str) -> dict:
    return json.loads(redis_con.get(decorate_key(key)))


def exists(key: str) -> bool:
    return bool(redis_con.exists(decorate_key(key)))
    ...
    while True:
        ...
        if msg.routing_key[:10] == 'submit.sm.' and msg.routing_key[:15] != 'submit.sm.resp.':
            ...
            qmsg = {
                ...
                'destination_addr': pdu.params['destination_addr'].decode(),
                'source_addr': pdu.params['source_addr'].decode(),
                'pdu_count': pdu_count,
                'binary_message': binary_message.decode(),
                'log_time': str(log_time)
            }

            push(props['message-id'], qmsg, ex=EXPIRY_PERIOD)

            if qmsg['source_addr'] is None:
                qmsg['source_addr'] = ''

            # We should only save the log when we have received the last message from a multipart
            # For HTTP multipart messages it is contained in 1 x PDU so specific to SMPP
            ...
            # It's a dlr
            if not exists(props['message-id']):
...
if __name__ == "__main__":
    print(f"Starting SMS CDR Consumer with Redis: {REDIS_HOST}:{REDIS_PORT}, "
          f"MySQL: {MYSQL_HOST}:{MYSQL_PORT}, AMQP: {AMQP_HOST}:{AMQP_PORT}")
    
    spec = txamqp.spec.load(AMQP_SPEC_FILE)
    ...
    d = ClientCreator(reactor,
        ...
        vhost=AMQP_VHOST,
        spec=spec).connectTCP(AMQP_HOST, AMQP_PORT)
    d.addCallback(gotConnection, AMQP_USER, AMQP_PASSWORD)
